...
def main():
    ...
    if reader_context:
        with reader_context as reader:
            ...
            hide_rules = {'task1/src/': 'hide_files_warn_pyc', 'task1/src/': 'hide_files_warn_pyc'}
...
def prediction_test():
    ...
    temp_path = False
    project_path = input_path = sys.argv[1]
    if os.path.isfile(input_path):
        zip_file_name = zipfile.ZipFile(input_path)
        project_path = 'tmp/'
        temp_path = True
        zip_file_name.extractall(project_path)

    if os.path.exists(project_path+'/task1'):
        try:
            sys.path.append(project_path)
            path_to_demo = os.path.join(os.getcwd(), sys.argv[2])
            path_to_weather = ''
            if len(sys.argv) > 3:
                path_to_weather = os.path.join(os.getcwd(), sys.argv[3])
            if not os.path.exists(path_to_demo):
                append_error('demo file not found in %s' % path_to_demo)
            elif path_to_weather and not os.path.exists(path_to_weather):
                append_error('weather file not found in %s' % path_to_weather)
            else:
                flight_prediction_test(project_path, path_to_demo, path_to_weather)
    ...
    if os.path.exists(project_path+'/task2'):
        try:
            sys.path.append(project_path)
            path_to_demo = os.path.join(os.getcwd(), sys.argv[2])
            if not os.path.exists(path_to_demo):
                ...
                github_classification_test(project_path, path_to_demo)
        except Exception as e:
            append_error(e)
        finally:
            sys.path.remove(project_path)
        ...
        append_error('Implementation for either Task 1 or Task 2 is missing')

def flight_prediction_test(project_path, path_to_demo, path_to_weather):
    import pandas as pd
    demo_test = pd.read_csv(path_to_demo)
    m_demo = len(demo_test)

    _tmp = __import__('task1.src', fromlist=['model'])

    fp = _tmp.model.FlightPredictor(path_to_weather)
    y_hat = fp.predict(demo_test)
    if not isinstance(y_hat, pd.DataFrame):
        append_error('Return type is not DataFrame')
    elif y_hat.shape != (m_demo, 2):
        append_error(f'Wrong Dimensions. Got {y_hat.shape}, expected {(m_demo, 2)}')


def github_classification_test(project_path, path_to_demo):
    import numpy as np
    import pandas as pd

    demo_test = pd.read_csv(path_to_demo).values.flatten()
    m_demo = demo_test.size

    _tmp = __import__('task2.src', fromlist=['model'])
    ghc = _tmp.model.GitHubClassifier()
    y_hat = ghc.classify(demo_test)
    if not isinstance(y_hat, np.ndarray):
        append_error('Return type is not a numpy array')
    elif y_hat.shape != (m_demo,):
        append_error(f'Wrong Dimensions. Got {y_hat.shape}, expected {(m_demo,)}')
